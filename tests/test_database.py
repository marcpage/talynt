...
from talynt.database import Join, Connection
...
def test_Threadsafe():
    with tempfile.TemporaryDirectory() as workspace:
        ...
        db = Connection.connect(f"sqlite://{db_path}?threadsafe=true")
...
def test_Sqlite():
    with tempfile.TemporaryDirectory() as workspace:
        db_path = os.path.join(workspace, "test.sqlite3")
        db = Connection.connect(f"sqlite://{db_path}?threadsafe=false")
        db.create_table("user",
            id="INTEGER PRIMARY KEY",
            name="VARCHAR(50)",
            email="VARCHAR(50)",
...
def test_as_objects_default_off():
    with tempfile.TemporaryDirectory() as workspace:
        db_path = os.path.join(workspace, "test.sqlite3")
        db = Connection.connect(f"sqlite://{db_path}?threadsafe=false", default_return_objects=False)
        db.create_tables(
            user={
                "id": "INTEGER PRIMARY KEY",
                "name": "VARCHAR(50)",
